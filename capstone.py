...
if selected_option == "Dashboard Harian":
    ...
    lca1, lca2, lca3 = st.columns([1, 1, 8])
    df['Tanggal'] = pd.to_datetime(df['Tanggal'])
    ...
    bpa1, bpa2, bpa3 = st.columns([1, 1, 8])
    ...
    with bpa2:
        ...
        bar_chart_aqi = alt.Chart(avg_aqi_per_province).mark_bar().encode(
            x=alt.X('Provinsi:N', sort='-y', axis=alt.Axis(labelAngle=-90)),
    ...
    narasi_empat = """
    <p style='text-align: justify; text-indent: 50px;'>Dari 21 provinsi yang diamati, sebanyak 12 provinsi memiliki status kualitas udara "Moderat". Ini menunjukkan bahwa sebagian besar provinsi memiliki kualitas udara yang dapat diterima, meskipun masih memerlukan pemantauan dan kewaspadaan terhadap polusi udara.
    Sebanyak 7 provinsi lainnya memiliki status kualitas udara "Baik". Ini menunjukkan bahwa sebagian besar provinsi memiliki kualitas udara yang baik dan relatif bersih, dengan dampak terhadap kesehatan yang minimal.
